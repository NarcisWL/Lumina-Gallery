...
const unhashToken = (value: string) => decodeURIComponent(value);
...
const normalizeFolderPath = (raw: string): string => {
  if (!raw) return '';
  const decoded = unhashToken(raw);
  const unixPath = decoded.replace(/\\\\/g, '/').replace(/\\/g, '/');
  const hasWindowsDrive = /^[a-zA-Z]:/.test(unixPath);
...
export const parseGalleryUrl = (input: string): GalleryLocation => {
  ...
  if (folderInHash !== null) {
    location = {
      ...location,
      folderPath: normalizeFolderPath(unhashToken(folderInHash)),
      view: 'folders',
    };
  }
